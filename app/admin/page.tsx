...
export default async function AdminPage() {
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-12 sm:px-6">
      ...
      <p className="mt-1 font-body text-sm text-brand-navy/60">
        ...
        {stats.map((stat) => {
          const Card = (
            <div className="flex flex-col gap-2 rounded-2xl border-2 border-brand-navy/10 bg-white p-5">
              <stat.icon size={18} className="text-brand-pink" />
              <span className="font-heading text-2xl font-semibold text-brand-navy">
                {stat.value ?? "—"}
              </span>
              <span className="font-body text-xs text-brand-navy/60">{stat.label}</span>
            </div>
          );
          return stat.href ? (
      ...
      <div className="mt-10 flex flex-wrap gap-3">
        <Link
          ...
          className="btn-soft rounded-full bg-brand-pink px-5 py-2.5 font-body text-sm font-semibold text-white"
        >
          + New Product
        </Link>
        <Link
